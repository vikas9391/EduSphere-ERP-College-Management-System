...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _set(TextEditingController c, dynamic v) => c.text = v?.toString() ?? '';
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  bool loading = true;
  @override void initState() { super.initState(); _load(); }
  Future<void> _load() async { try { items = await ApiService.instance.list('/announcements'); } catch (e) { if (mounted) snack(context, cleanError(e)); } if (mounted) setState(() => loading = false); }
  Future<void> _read(Map<String, dynamic> x) async { final id = x['id']; if (id == null || x['read'] == true) return; try { await ApiService.instance.request('/announcements/$id/read', method: 'POST'); if (mounted) setState(() => x['read'] = true); } catch (e) { if (mounted) snack(context, cleanError(e)); } }
  Future<void> _readAll() async { try { await ApiService.instance.request('/announcements/read-all', method: 'POST'); if (mounted) setState(() { for (final x in items) { x['read'] = true; } }); } catch (e) { if (mounted) snack(context, cleanError(e)); } }
  @override Widget build(BuildContext context) {
    final unread = items.where((x) => x['read'] != true).length;
    return Scaffold(appBar: AppBar(title: const Text('Notifications'), actions: [if (unread > 0) TextButton(onPressed: _readAll, child: const Text('Read all'))]), body: loading ? const Center(child: CircularProgressIndicator()) : RefreshIndicator(onRefresh: _load, child: items.isEmpty ? ListView(children: const [SizedBox(height: 180), Center(child: Text('No notifications'))]) : ListView.builder(padding: const EdgeInsets.all(16), itemCount: items.length, itemBuilder: (context, i) { final x = items[i]; final read = x['read'] == true; return Card(color: read ? null : const Color(0xFFE8F5E9), child: ListTile(onTap: () => _read(x), leading: CircleAvatar(child: Icon(read ? Icons.notifications_none : Icons.notifications)), title: Text('${x['title'] ?? 'Announcement'}', style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${x['message'] ?? ''}'), trailing: read ? null : const Icon(Icons.circle, size: 10)); }))); }
}
