    ...
    return Scaffold(
      ...
      body: loading ? const Center(child: CircularProgressIndicator()) : RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.all(20), children: [
          ...
          const SizedBox(height: 20), SizedBox(height: 52, child: FilledButton.icon(onPressed: saving ? null : _save, icon: const Icon(Icons.save_outlined), label: Text(saving ? 'Saving…' : 'Save profile'))),
        ]),
      ),
    );
  }
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _readAll() async {
    try {
      ...
      if (mounted) setState(() { for (final item in items) item['read'] = true; });
    } catch (e) { if (mounted) snack(context, cleanError(e)); }
  }
