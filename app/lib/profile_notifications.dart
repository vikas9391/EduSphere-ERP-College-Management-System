...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  @override
  void dispose() {
    for (final c in [phone, address, city, state, pincode, parentPhone, parentEmail, bloodGroup]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _load() async {
    try {
      ...
      _set(phone, profile['phone']);
      _set(address, profile['address']);
      _set(city, profile['city']);
      _set(state, profile['state']);
      _set(pincode, profile['pincode']);
      _set(parentPhone, profile['parentPhone']);
      _set(parentEmail, profile['parentEmail']);
      _set(bloodGroup, profile['bloodGroup']);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    if (!widget.role.toUpperCase().contains('STUDENT')) {
      return Scaffold(
        ...
        body: const Center(child: Text('Profile management is currently available for students.')),
      );
    }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('${profile['courseName'] ?? 'Course'} · ${profile['department'] ?? 'Department'}'),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _section(String title, List<TextEditingController> controllers, List<String> labels) {
    return Padding(
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _load() async {
    try {
      items = await ApiService.instance.list('/announcements');
    } catch (e) {
      if (mounted) snack(context, cleanError(e));
    } finally {
      if (mounted) setState(() => loading = false);
  ...
  Future<void> _read(Map<String, dynamic> item) async {
    ...
    try {
      await ApiService.instance.request('/announcements/$id/read', method: 'POST');
  ...
  Future<void> _readAll() async {
    try {
      await ApiService.instance.request('/announcements/read-all', method: 'POST');
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (unread > 0)
            TextButton(onPressed: _readAll, child: const Text('Read all')),
        ],
      ),
      body: loading
          ? const Center(child: CircularProgressIndicator())
              ...
              child: items.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          color: read ? null : const Color(0xFFE8F5E9),
                          child: ListTile(
                            onTap: () => _read(item),
                            leading: CircleAvatar(
                              child: Icon(read ? Icons.notifications_none : Icons.notifications),
                            ),
                            title: Text(
