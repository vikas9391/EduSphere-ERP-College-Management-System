        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              final x = rows[index];
              final date = '${x['examDate'] ?? '—'}'; final start = '${x['startTime'] ?? ''}'; final end = '${x['endTime'] ?? ''}';
              final time = start.isEmpty && end.isEmpty ? 'Time not specified' : '$start${end.isEmpty ? '' : ' – $end'}';
              return Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.event_note_outlined)), title: Text('${x['subjectName'] ?? 'Subject'}', style: const TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('${x['examName'] ?? 'Exam'}\n$date · $time\nRoom: ${x['room'] ?? '—'}'), isThreeLine: true, trailing: x['maxMarks'] == null ? null : Text('${x['maxMarks']} marks', style: const TextStyle(fontWeight: FontWeight.w700))));
            },
          ),
        );
      },
