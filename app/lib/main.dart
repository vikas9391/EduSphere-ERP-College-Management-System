...
class ApiService {
  ...
  Future<Map<String, dynamic>> login(String collegeCode, String username, String password) async {
    final data = await _request('/auth/login', method: 'POST', body: {'collegeCode': collegeCode, 'username': username, 'password': password}, retry: false);
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('accessToken', data['accessToken'] ?? '');
  ...
  Future<void> logout() async { final p = await SharedPreferences.getInstance(); await p.remove('accessToken'); await p.remove('refreshToken'); await p.remove('role'); await p.remove('username'); }
...
class _RootScreenState extends State<RootScreen> {
  ...
  Future<void> _restore() async { final p = await SharedPreferences.getInstance(); final token = p.getString('accessToken'); setState(() { role = token == null || token.isEmpty ? null : p.getString('role'); loading = false; }); }
  void signedIn(String r) => setState(() => role = r);
  Future<void> signOut() async { await ApiService.instance.logout(); if (mounted) setState(() => role = null); }
  @override Widget build(BuildContext context) { if (loading) return const SplashScreen(); if (role == null) return LoginScreen(onSignedIn: signedIn); return HomeScreen(role: role!, onLogout: signOut); }
}
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> submit() async {
    if (college.text.trim().isEmpty || user.text.trim().isEmpty || pass.text.isEmpty) { _error('Enter your college code, username and password.'); return; }
