...
async function rawRequest<T>(path: string, options: RequestInit = {}, token?: string): Promise<{ response: Response; data: T | ApiError | null }> {
...
async function refreshAccessToken(): Promise<string | null> {
  ...
  try {
    const { response, data } = await rawRequest<LoginResponse>('/auth/refresh', {
      method: 'POST',
      body: JSON.stringify({ refreshToken }),
    });
    if (!response.ok) throw new Error(errorMessage(data, response.status));

    const result = unwrap<LoginResponse>(data);
    if (!result.accessToken) throw new Error('Refresh response did not contain an access token.');
    ...
    await AsyncStorage.multiRemove(['accessToken', 'refreshToken', 'userRole', 'username']);
    return null;
  }
}
...
async function request<T>(path: string, options: RequestInit = {}, retry = true): Promise<T> {
  ...
  const { response, data } = await rawRequest<T>(path, options, token ?? undefined);
