...
public class StudentAssignmentService {
    ...
    public StudentAssignmentService(ClassEnrollmentRepository classEnrollmentRepository,
    ...
    public List<StudentAssignmentResponse> getAssignments(Long studentId) {
        ...
        List<Long> classSubjectIds = enrollments.stream()
                ...
                .distinct()
                .toList();
        if (classSubjectIds.isEmpty()) return List.of();
        ...
        Map<Long, AssignmentSubmission> submissionsByAssignmentId = submissionRepository.findByStudentId(studentId)
                .stream()
                .collect(Collectors.toMap(s -> s.getAssignment().getId(), Function.identity(), (first, ignored) -> first));

        return assignments.stream().map(a -> map(a, submissionsByAssignmentId.get(a.getId()))).toList();
    }

    private StudentAssignmentResponse map(Assignment assignment, AssignmentSubmission submission) {
        var cs = assignment.getClassSubject();
        ...
        if (submission == null) return builder.submissionStatus(NOT_SUBMITTED).build();
        return builder.submissionStatus(submission.getStatus() != null ? submission.getStatus() : "SUBMITTED")
