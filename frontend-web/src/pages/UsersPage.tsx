  ...
  return (
    <Layout>
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-medium text-ink">Staff accounts</h1>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-md bg-brass px-4 py-2 text-sm font-medium text-white hover:bg-brass-bright disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-dim">No staff accounts yet.</p>
      ) : (
        <div className="overflow-hidden rounded-lg border border-parchment-line bg-white/50">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-parchment-line last:border-0">
                  <td className="px-4 py-3 text-ink">
                    <div className="flex items-center gap-2">
                      <UserCircle2 size={16} className="text-slate-dim" />
                      {u.firstName} {u.lastName}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-dim">{u.email}</td>
                  <td className="px-4 py-3 font-mono text-xs text-ink">{u.roleName}</td>
