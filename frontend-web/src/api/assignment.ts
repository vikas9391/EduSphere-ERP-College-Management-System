...
/** Student identity is derived from the authenticated principal on the backend. */
export interface AssignmentSubmissionPayload {
  assignmentId: number
  submissionUrl: string
}

export async function submitAssignment(payload: AssignmentSubmissionPayload) {
  const res = await api.post<AssignmentSubmission>('/submissions', payload)
  return res.data
}

export async function getSubmissions() {
  const res = await api.get<AssignmentSubmission[]>('/submissions')
  return res.data
}

export async function getAssignmentSubmissions(assignmentId: number) {
  const res = await api.get<AssignmentSubmission[]>(`/submissions/assignment/${assignmentId}`)
  return res.data
}

export async function evaluateSubmission(id: number, marks: number, feedback: string) {
  const res = await api.put<AssignmentSubmission>(`/submissions/${id}/evaluate`, null, {
